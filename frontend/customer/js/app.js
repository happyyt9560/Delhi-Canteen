...
function checkout() {
    ...
    const settings = bookingChargeSettings(), onlineSettings = onlinePaymentSettings(), pickupShop = shopSettings(), pickupAddress = `${pickupShop.name || 'Delhi Canteen'} — ${pickupShop.address || ''}`, minimumShortfall = minimumOrderShortfall(totals.total), minimumAmount = Number(minimumOrderConfig().amount) || 0;
    root.innerHTML = `<section class="page-heading"><p class="eyebrow">ONE LAST STEP</p><h1>Checkout</h1></section><div class="checkout-layout"><form id="checkout-form" class="checkout-form"><h2>Delivery details</h2><label>Customer name<input required name="name" autocomplete="name" placeholder="Your full name" value="${user.name || ''}"></label><label>Phone number<input required name="phone" type="tel" inputmode="numeric" autocomplete="tel" pattern="[0-9]{10}" title="Enter a 10-digit mobile number" placeholder="10-digit mobile number" value="${user.phone || ''}"></label><label>Payment mode<select name="mode" id="payment-mode"><option>Cash on Delivery</option><option>Online Payment</option><option>Self Take (Store Pickup)</option></select></label><div id="address-fields"><label>Complete address<textarea required name="address" autocomplete="street-address" placeholder="House no., street, locality, Delhi">${user.address || ''}</textarea></label></div><div class="pickup-box" hidden><b>Store Pickup Address</b><p>${pickupShop.name || 'Delhi Canteen'}<br>${pickupShop.address || 'Address not available'}<br><b>Mobile:</b> ${pickupShop.phone || 'Not available'}</p></div>${minimumShortfall?`<p class="booking-checkout-note minimum-order-notice" role="alert"><b>Minimum order value is ${money(minimumAmount)}.</b> Your Item total is ${money(totals.total)}; add items worth ${money(minimumShortfall)} more to place this order.</p>`:''}<div id="booking-charge-area"></div><button class="primary wide" type="submit">Place order →</button></form><aside><h2>Order summary</h2>${totals.items.map(item => `<p class="mini-item">${item.p.name} × ${item.qty}<b>${money(item.p.price * item.qty)}</b></p>`).join('')}<div id="checkout-summary"></div></aside></div>`;
    ...
    form.onsubmit = event => {
        ...
        const shortfall = minimumOrderShortfall(totals.total);
        if (shortfall) { toast(`Minimum order value is ${money(minimumOrderConfig().amount)}. Please add ${money(shortfall)} more to your Item total.`); return; }
...
function bookingChargeCheckout() {
    ...
    $('#booking-charge-form').onsubmit = event => {
        ...
        const itemTotal = (order.items || []).reduce((sum, item) => sum + (Number(item.product?.price ?? item.price) || 0) * (Number(item.qty ?? item.quantity) || 0), 0), shortfall = minimumOrderShortfall(itemTotal);
        if (shortfall) { toast(`Minimum order value is ${money(minimumOrderConfig().amount)}. Please add ${money(shortfall)} more to your Item total.`); return; }
...
// Minimum order is always checked against Item total, before fees or discounts.
const minimumOrderConfig=()=>({enabled:false,amount:0,...store.get('dc_minimum_order_settings',{})});
function minimumOrderShortfall(itemTotal){const config=minimumOrderConfig(),amount=Math.max(0,Number(config.amount)||0);return config.enabled&&amount>0&&itemTotal<amount?amount-itemTotal:0;}
...
window.addEventListener('storage',event=>{if(['dc_cod_settings','dc_booking_charge_settings','dc_online_payment_settings','dc_minimum_order_settings'].includes(event.key)&&$('#checkout-page'))checkout();});
